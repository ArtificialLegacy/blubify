...
function SignupForm() {
  const [status, setStatus] = useState(SignupStatus.Success)
  ...
  const formik = useFormik({
    ...
    onSubmit: async (_values: SignupData) => {
      ...
      if (response.status === SignupStatus.SuccessNoSession)
        window.location.pathname = '/login'

      setStatus(response.status as SignupStatus)
    },
  ...
  return (
    <Box>
      ...
      <Typography
        ...
        sx={{
          visibility:
            status === SignupStatus.InvalidRequest ? 'visible' : 'hidden',
        }}
      >
