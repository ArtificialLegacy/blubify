 ...
 *
 * @example
 * const result = await movePlaylistUp(playlistId)
 *
 * movePlaylistUp(playlistId).then((_result) => console.log(_result))
 */
async function submitPlaylistOrderUp(_playlistId: string): Promise<APIStatus> {
  const [signal, clear] = netTimeout()
  let result: APIStatus

  try {
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/api/playlist/edit/order/up/${_playlistId}`,
      {
        method: 'PATCH',
        headers: {
